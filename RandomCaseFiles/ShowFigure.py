...
# 读取CSV文件
df = pd.read_csv('comp_runtime.csv')

# 创建一个包含两个子图的画布
fig, (upper_ax, middle_ax, lower_ax) = plt.subplots(3, 1, figsize=(16, 8), sharex=True)
...
# 绘制基准线
upper_ax.axhline(y=60, color='r', linestyle='--', label='Baseline (Runtime = 60s)')
...
upper_ax.set_ylabel('Runtime(s), Design Complexity, All-type Constraint (#)')
upper_ax.legend()
upper_ax.grid(True)
...
plt.tight_layout()
plt.savefig('Min_I_vs_Runtime.png', dpi=300, bbox_inches='tight')
